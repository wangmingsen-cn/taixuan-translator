"""
太玄智译 - OpenAI 翻译引擎
支持 GPT-4o / GPT-4o-mini 等模型，含重试和限流处理
"""
...
class OpenAITranslationEngine(BaseTranslationEngine):
    ...
    def _do_translate(
        ...
        """调用 OpenAI API 翻译"""
        from openai import APIStatusError, APITimeoutError  # type: ignore[import]
        ...
        for attempt in range(cfg.max_retries + 1):
            try:
                ...
                logger.debug(
                    f"[OpenAI] 翻译成功, tokens={response.usage.total_tokens if response.usage else 0}"
                )
                return translated.strip()

            except APITimeoutError as e:
                last_error = TranslationTimeoutError("OpenAI", cfg.timeout)
                logger.warning(f"[OpenAI] 请求超时 (attempt {attempt + 1}): {e}")
                ...
                if e.status_code == 429:
                    ...
                    last_error = TranslationRateLimitError("OpenAI", retry_after)
                    logger.warning(f"[OpenAI] 限流，{retry_after}s后重试")
                    if attempt < cfg.max_retries:
                        time.sleep(min(retry_after, 30))
                elif e.status_code == 401:
                    raise InvalidAPIKeyError("OpenAI") from e
                else:
                    last_error = TranslationAPIError("OpenAI", e.status_code, str(e))
                    logger.error(f"[OpenAI] API错误 {e.status_code}: {e}")
                    if attempt < cfg.max_retries:
                        time.sleep(2 ** attempt)

            except Exception as e:
                last_error = e
                logger.error(f"[OpenAI] 未知错误 (attempt {attempt + 1}): {e}")
                if attempt < cfg.max_retries:
                    time.sleep(2 ** attempt)

        raise last_error or TranslationAPIError("OpenAI")
