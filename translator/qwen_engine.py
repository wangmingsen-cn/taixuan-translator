...
class QwenTranslationEngine(BaseTranslationEngine):
    """通义千问翻译引擎（DashScope OpenAI 兼容接口）"""
    ...
    def _get_client(self) -> object:
        if self._client is None:
            ...
            cfg = self.settings.qwen
            if not cfg.api_key:
                raise InvalidAPIKeyError("Qwen")
    ...
    def validate_config(self) -> bool:
        return bool(self.settings.qwen.api_key)

    def get_model_name(self) -> str:
        return self.settings.qwen.model
    ...
    def _do_translate(self, text: str, source_language: str, target_language: str) -> str:
        ...
        try:
            ...
            err = str(e).lower()
            if "401" in err or "apikey" in err:
                raise InvalidAPIKeyError("Qwen") from e
            raise TranslationAPIError("Qwen", detail=str(e)) from e
